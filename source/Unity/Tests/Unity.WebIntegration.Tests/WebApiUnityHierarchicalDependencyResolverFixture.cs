        ...
        public void When_resolving_type_with_hierarchical_lifetime_then_returns_different_instance_in_each_scope()
        {
            using (var container = new UnityContainer())
            {
                ...
                using (var resolver = new UnityHierarchicalDependencyResolver(container))
                {
                    ...
                    Assert.NotNull(parentResolve);
                    Assert.NotNull(scope1Resolve1);
                    Assert.NotNull(scope1Resolve2);
                    Assert.NotNull(scope2Resolve1);
                    Assert.NotNull(scope2Resolve2);

                    Assert.NotSame(parentResolve, scope1Resolve1);
                    Assert.NotSame(parentResolve, scope2Resolve1);

                    Assert.NotSame(scope1Resolve1, scope2Resolve1);

                    Assert.Same(scope1Resolve1, scope1Resolve2);

                    Assert.Same(scope2Resolve1, scope2Resolve2);
                }
            }
        }
